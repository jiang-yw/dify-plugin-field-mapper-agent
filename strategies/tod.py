...
class TaskOrientedDialogueStrategy(AgentStrategy):
    ...
    def _build_mapping(self, field: DialogueField) -> dict[str, str]:
        mapping = {}

        for source, target in (field.mapping or {}).items():
            mapping[_normalize_text(source)] = str(target)
        ...
        for option in field.options or []:
            ...
            target = str(value)
            candidates = [value, option.get("label"), option.get("name")]
            ...
            for candidate in candidates:
                if candidate is not None:
                    mapping[_normalize_text(candidate)] = target

        return mapping

    def _normalize_field_value(self, field: DialogueField, value: Any) -> str:
        if value is None:
            return ""

        raw_value = str(value).strip()
        mapping = self._build_mapping(field)
        if not mapping:
            return raw_value
        ...
        for source, target in mapping.items():
            if source and (source in normalized_value or normalized_value in source):
                return target
        ...
        if is_valid or params.query.strip() != "":
            ...
            for field in self.dialogue_state.fields:
                if field.name in extracted_answers and extracted_answers[field.name]:
                    fields_to_update[field.name] = self._normalize_field_value(
                        field, extracted_answers[field.name]
                    )
                    answers_saved = True
